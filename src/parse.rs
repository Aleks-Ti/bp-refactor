...
fn delimited<Prefix, T, Suffix>(prefix_to_ignore: Prefix, dest_parser: T, suffix_to_ignore: Suffix) -> Delimited<Prefix, T, Suffix>
where
    Prefix: Parser,
    T: Parser,
    Suffix: Parser,
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        match self.parsers.0.parse(input.clone()) {
            ...
            Err(()) => self
                ...
                .1
                .parse(input)
                .and_then(|(remaining, a1)| self.parsers.0.parse(remaining).map(|(remaining, a0)| (remaining, (a0, a1)))),
        }
    }
}
        ...
        match self.parsers.0.parse(input.clone()) {
            Ok((remaining, a0)) => match self.parsers.1.parse(remaining.clone()) {
                ...
                Err(()) => self
                    ...
                    .and_then(|(remaining, a2)| self.parsers.1.parse(remaining).map(|(remaining, a1)| (remaining, (a0, a1, a2)))),
            },
            Err(()) => match self.parsers.1.parse(input.clone()) {
                Ok((remaining, a1)) => {
                    match self.parsers.0.parse(remaining.clone()) {
                        Ok((remaining, a0)) => self.parsers.2.parse(remaining).map(|(remaining, a2)| (remaining, (a0, a1, a2))),
                        Err(()) => self.parsers.2.parse(remaining.clone()).and_then(|(remaining, a2)| {
                            self.parsers.0.parse(remaining).map(|(remaining, a0)| (remaining, (a0, a1, a2)))
                        }),
                    }
                }
                Err(()) => {
                    self.parsers
                        .2
                        .parse(input.clone())
                        .and_then(|(remaining, a2)| match self.parsers.0.parse(remaining.clone()) {
                            Ok((remaining, a0)) => self.parsers.1.parse(remaining).map(|(remaining, a1)| (remaining, (a0, a1, a2))),
                            Err(()) => self.parsers.1.parse(remaining.clone()).and_then(|(remaining, a1)| {
                                self.parsers.0.parse(remaining).map(|(remaining, a0)| (remaining, (a0, a1, a2)))
                            }),
                        })
                }
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
fn alt4<Dest, A0: Parser<Dest = Dest>, A1: Parser<Dest = Dest>, A2: Parser<Dest = Dest>, A3: Parser<Dest = Dest>>(
    a0: A0,
    a1: A1,
    a2: A2,
    a3: A3,
...
impl Parsable for AppLogJournalKind {
    type Parser = Preceded<
        ...
        Alt<(
            Map<
                Preceded<StripWhitespace<Tag>, Delimited<Tag, Permutation<(KeyValue<Unquote>, KeyValue<stdp::U32>)>, Tag>>,
            ...
            Map<
                Preceded<
                    StripWhitespace<Tag>,
                    Delimited<Tag, Permutation<(KeyValue<Unquote>, KeyValue<Unquote>, KeyValue<stdp::U32>)>, Tag>,
                >,
                fn((String, String, u32)) -> AppLogJournalKind,
            >,
            Map<
                Preceded<StripWhitespace<Tag>, Delimited<Tag, Permutation<(KeyValue<Unquote>, KeyValue<Unquote>)>, Tag>>,
            ...
            alt8(
                map(
                    preceded(
                        ...
                        delimited(
                            tag("{"),
                            permutation2(key_value("user_id", unquote()), key_value("authorized_capital", stdp::U32)),
                            tag("}"),
                        ),
                    ),
                ...
                map(
                    preceded(
                        ...
                        delimited(
                            tag("{"),
                            permutation2(key_value("asset_id", unquote()), key_value("user_id", unquote())),
                            tag("}"),
                        ),
                    ),
                    |(asset_id, user_id)| AppLogJournalKind::UnregisterAsset { asset_id, user_id },
                ),
                map(preceded(strip_whitespace(tag("DepositCash")), UserCash::parser()), |user_cash| {
                    AppLogJournalKind::DepositCash(user_cash)
                }),
                map(preceded(strip_whitespace(tag("WithdrawCash")), UserCash::parser()), |user_cash| {
                    AppLogJournalKind::DepositCash(user_cash)
                }),
                map(preceded(strip_whitespace(tag("BuyAsset")), UserBacket::parser()), |user_backet| {
                    AppLogJournalKind::BuyAsset(user_backet)
                }),
                map(preceded(strip_whitespace(tag("SellAsset")), UserBacket::parser()), |user_backet| {
                    AppLogJournalKind::SellAsset(user_backet)
                }),
            ),
        )
    }
...
impl Parsable for LogLine {
    type Parser = Map<All<(<LogKind as Parsable>::Parser, StripWhitespace<Preceded<Tag, stdp::U32>>)>, fn((LogKind, u32)) -> Self>;
    fn parser() -> Self::Parser {
        map(
            all2(LogKind::parser(), strip_whitespace(preceded(tag("requestid="), stdp::U32))),
...
mod test {
    ...
    fn test_asset_dsc() {
        assert_eq!(
            all2(strip_whitespace(tag("AssetDsc")), strip_whitespace(tag("{"))).parse(" AssetDsc { ".into()),
            Ok(("".into(), ((), ())))
        );
        ...
        assert_eq!(
            AssetDsc::parser().parse(r#" AssetDsc { "id" : "usd" , "dsc" : "USA dollar" , } nice "#.into()),
            Ok((
                "nice ".into(),
                AssetDsc {
                    id: "usd".into(),
    ...
    fn test_log_kind() {
        assert_eq!(
            preceded(strip_whitespace(tag("NetworkError")), strip_whitespace(unquote())).parse(r#"NetworkError "url unknown""#.into()),
        ...
        assert_eq!(
            ...
            Ok((
                "".into(),
                LogKind::System(SystemLogKind::Error(SystemLogErrorKind::NetworkError("url unknown".into())))
            ))
        );
        ...
        assert_eq!(
            LogKind::parser().parse(r#"App::Journal CreateUser {"user_id": "Steeve", "authorized_capital": 10000,}"#.into()),
            Ok((
                "".into(),
        ...
        assert_eq!(
            ...
            Ok((
                "".into(),
                LogKind::App(AppLogKind::Journal(AppLogJournalKind::DeleteUser { user_id: "Steeve".into() }))
            ))
        );
        assert_eq!(
            LogKind::parser()
                .parse(r#"App::Journal RegisterAsset {"asset_id": "bayc", "liquidity": 100000000, "user_id": "Steeve",}"#.into()),
            Ok((
                "".into(),
        ...
        assert_eq!(
            LogKind::parser().parse(r#"App::Journal DepositCash UserCash{"user_id": "Steeve", "count": 10,}"#.into()),
            Ok((
                "".into(),
        ...
        assert_eq!(
            LogKind::parser().parse(
                r#"App::Journal BuyAsset UserBacket{"user_id": "Steeve", "backet": Backet{"asset_id":"bayc","count":1,},}"#.into()
            ),
            Ok((
                "".into(),
